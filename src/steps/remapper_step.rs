...
impl Processable for ProcessStep {
    ...
    async fn process(
        ...
        ProcessorError,
    > {
        // In-memory map to track listings placed within this batch:
        let mut batch_listing_map: HashMap<String, String> = HashMap::new(); // token_data_id -> listing_id

        // Create tuples to collect both activities and listings
        let results: Vec<(
            Vec<NftMarketplaceActivity>,
            Vec<CurrentNFTMarketplaceListing>,
            Vec<CurrentNFTMarketplaceTokenOffer>,
