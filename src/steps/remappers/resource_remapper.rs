                ...
                if let Some(change) = wsc.change.as_ref() {
                    if let write_set_change::Change::WriteResource(write_resource) = change {
                        ...
                        if let Some(object_core) = get_object_core(move_resource_type, &data) {
                            // info!("Found ObjectCore at address: {}", move_resource_address);
                        ...
                        if let Some((_marketplace_name, resource_config)) = self
                            ...
                            )
                        {
                            ...
                            match resource_config.resource_action {
                                ...
                                ResourceType::FixedPriceListing => {
                                    info!("Found fixed price listing resource config");
                        ...
                        debug!("Skipping non-WriteResource change");
                    }
                }
            }
                            ...
                            match resource_config.resource_action {
                                ResourceType::Listing => {
                                        ...
                                        .get(&move_resource_address)
                                    {
                                        ...
                                        None => {
                                            error!("ERROR: Expected listing metadata for address {} but none was found", move_resource_address);
                                            return Err(ProcessorError::ProcessError {
                                                message: format!(
